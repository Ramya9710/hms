...
public class InPatientBO {
    ...
    public InPatient createIp(Patients patients, Map<Long, InPatient> inPatientMap, Map<Long, Bed> bedMap) throws Exception {
        ...
        try {
            ...
            if (!inPatientMap.isEmpty() && inPatientMap.containsKey(inPatientMap.get(patients.getPatientType()))) {
    ...
    public Patients allocateBedForIpPatients(Patients patients, Map<Long, Bed> bedMap) throws Exception {
        ...
        if (inPatient.getPatients() != null && inPatient.getPatients().getPatientType() == patients.getPatientType())
            if (patients.getPatientType().equals("InPatients"))
                if (patients.equals(inPatient.getPatients().getPatientType())) {
